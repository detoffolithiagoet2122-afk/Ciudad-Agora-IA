...
// Valida y guarda una imagen subida en api/uploads/<subdir>/, devolviendo
// la ruta relativa a guardar en la base. Corta la ejecución con fail() si
// algo no está bien (archivo faltante, muy pesado, o no es una imagen real).
function handleImageUpload(string $fieldName, string $subdir): string {
    if (!isset($_FILES[$fieldName]) || $_FILES[$fieldName]['error'] !== UPLOAD_ERR_OK) {
        fail('No pudimos recibir la imagen. Probá con otro archivo.');
    }
    $file = $_FILES[$fieldName];
    $maxBytes = 3 * 1024 * 1024; // 3MB
    if ($file['size'] > $maxBytes) fail('La imagen es muy pesada (máximo 3MB).');

    $info = @getimagesize($file['tmp_name']);
    if ($info === false) fail('Ese archivo no es una imagen válida.');
    $allowed = [IMAGETYPE_JPEG => 'jpg', IMAGETYPE_PNG => 'png', IMAGETYPE_WEBP => 'webp', IMAGETYPE_GIF => 'gif'];
    if (!isset($allowed[$info[2]])) fail('Usá una imagen JPG, PNG, WEBP o GIF.');
    $ext = $allowed[$info[2]];
    ...
    if (!move_uploaded_file($file['tmp_name'], $dir . '/' . $filename)) {
        fail('No pudimos guardar la imagen en el servidor.');
    }
    return 'uploads/' . $subdir . '/' . $filename;
}

// Avisa a todos los miembros de un grupo (menos a quien hizo la acción)
// que pasó algo nuevo. Versión simple: una fila por persona, se marca
// leída cuando abren la campanita.
function notifyGroupMembers(PDO $pdo, int $groupId, int $actorId, string $type, string $message): void {
    $st = $pdo->prepare('SELECT user_id FROM group_members WHERE group_id=? AND user_id<>?');
    $st->execute([$groupId, $actorId]);
    $ins = $pdo->prepare('INSERT INTO notifications (user_id, group_id, type, actor_id, message) VALUES (?,?,?,?,?)');
    foreach ($st->fetchAll(PDO::FETCH_COLUMN) as $memberId) {
        $ins->execute([$memberId, $groupId, $type, $actorId, $message]);
    }
}
...
// Envía cualquier mail usando la API de Brevo (por HTTPS, puerto 443),
// porque InfinityFree bloquea mail() y las conexiones SMTP salientes.
function sendBrevoEmail(string $to, string $toName, string $subject, string $htmlBody): bool {
    $payload = [
...
// Arma el HTML de un mail con el mismo look (botón rosa, etc.) para no
// repetir el mismo bloque de estilos en cada tipo de mail que mandamos.
function emailTemplateHTML(string $name, string $introHtml, string $link, string $buttonText, string $footNoteHtml = ''): string {
    return '<!DOCTYPE html><html><head><meta charset="UTF-8">'
        ...
        . '<p>' . htmlspecialchars($introHtml, ENT_QUOTES, 'UTF-8') . '</p>'
        ...
        . 'font-weight:bold;padding:12px 22px;border-radius:4px;margin:10px 0">' . htmlspecialchars($buttonText, ENT_QUOTES, 'UTF-8') . '</a></p>'
        . '<p style="font-size:12px;color:#666">Si el botón no funciona, copiá y pegá este link en el navegador:<br>'
        . htmlspecialchars($link, ENT_QUOTES, 'UTF-8') . '</p>'
        . ($footNoteHtml !== '' ? '<p style="font-size:12px;color:#666">' . htmlspecialchars($footNoteHtml, ENT_QUOTES, 'UTF-8') . '</p>' : '')
        . '</div></body></html>';
}
...
function sendVerificationEmail(string $to, string $name, string $link): bool {
    $html = emailTemplateHTML(
        $name,
        'Gracias por crear tu cuenta en Tablón. Para poder entrar, confirmá tu mail tocando el botón de abajo (el link vale por 24 horas):',
        $link,
        'Confirmar mi cuenta',
        'Si vos no creaste esta cuenta, podés ignorar este mensaje.'
    );
    return sendBrevoEmail($to, $name, 'Confirma tu cuenta de Tablon', $html);
}

function sendPasswordResetEmail(string $to, string $name, string $link): bool {
    $html = emailTemplateHTML(
        $name,
        'Pediste restablecer tu contraseña en Tablón. Tocá el botón de abajo para elegir una nueva (el link vale por 1 hora):',
        $link,
        'Elegir nueva contraseña',
        'Si vos no pediste esto, podés ignorar este mensaje: tu contraseña actual sigue funcionando igual.'
    );
    return sendBrevoEmail($to, $name, 'Restablecer tu contrasena de Tablon', $html);
}
...
if (empty($input) && !empty($_POST)) {
    // Subida de archivos (multipart/form-data): no viene como JSON en el
    // body, viene en $_POST + $_FILES.
    $input = $_POST;
}
...
try {
    ...
    switch ($action) {
        ...
        case 'bootstrap': {
            ...
            $users = $pdo->query('SELECT id, name, handle, color_idx, avatar_path FROM users ORDER BY id')->fetchAll();
            ...
            $groups = $pdo->query('SELECT id, name, category_id, zone, description, photo_path FROM `groups` ORDER BY id')->fetchAll();
            ...
            // Últimas notificaciones de actividad de grupo del usuario actual.
            $notifications = [];
            $unreadCount = 0;
            if ($cuid !== null) {
                $notSt = $pdo->prepare('SELECT id, group_id, type, message, is_read, created_at FROM notifications WHERE user_id=? ORDER BY id DESC LIMIT 30');
                $notSt->execute([$cuid]);
                foreach ($notSt->fetchAll() as $n) {
                    $notifications[] = [
                        'id' => (string)$n['id'],
        ...
        case 'request_password_reset': {
            $email = strtolower(trim((string)($input['email'] ?? '')));
            if ($email === '' || !preg_match('/^[^@\s]+@[^@\s]+\.[^@\s]+$/', $email)) fail('Ingresá un mail válido.');

            $st = $pdo->prepare('SELECT id, name FROM users WHERE LOWER(email) = ?');
            $st->execute([$email]);
            $u = $st->fetch();

            $devLink = null;
            // Respondemos siempre "ok" exista o no la cuenta, para no
            // revelar qué mails están registrados en la app.
            if ($u) {
                $token = bin2hex(random_bytes(20));
                ...
                $pdo->prepare('UPDATE users SET password_reset_token=?, password_reset_expires=? WHERE id=?')
                    ->execute([$token, $expires, $u['id']]);
                $link = rtrim(SITE_URL, '/') . '/index.html?reset=' . $token;
                sendPasswordResetEmail($email, $u['name'], $link);
                $devLink = APP_DEBUG ? $link : null;
            }
            ok(['sent' => true, 'devResetLink' => $devLink]);
            break;
        }

        // Confirma el código del mail y guarda la contraseña nueva.
        case 'reset_password': {
            $token = trim((string)($input['token'] ?? ''));
            $password = (string)($input['password'] ?? '');
            if ($token === '') fail('Falta el código de recuperación.');
            if (strlen($password) < 4) fail('La contraseña tiene que tener al menos 4 caracteres.');

            $st = $pdo->prepare('SELECT id, name, password_reset_expires FROM users WHERE password_reset_token = ?');
            $st->execute([$token]);
            $u = $st->fetch();
            if (!$u) fail('Ese link ya no es válido. Pedí uno nuevo.');
            if ($u['password_reset_expires'] !== null && strtotime($u['password_reset_expires']) < time()) {
                fail('Este link venció (valía 1 hora). Pedí uno nuevo.');
            }

            $hash = password_hash($password, PASSWORD_DEFAULT);
            $pdo->prepare('UPDATE users SET password_hash=?, password_reset_token=NULL, password_reset_expires=NULL, email_verified_at=COALESCE(email_verified_at, NOW()) WHERE id=?')
                ->execute([$hash, $u['id']]);
            ok(['name' => $u['name']]);
            break;
        }
        ...
        // Sube (o reemplaza) tu foto de perfil.
        case 'upload_avatar': {
            $uid = requireLogin();
            $path = handleImageUpload('photo', 'avatars');
        ...
        // Sube (o reemplaza) la foto del grupo. Solo un admin del grupo puede.
        case 'upload_group_photo': {
            $uid = requireLogin();
            $gid = (int)($input['groupId'] ?? 0);
            if (!isGroupAdmin($pdo, $gid, $uid)) fail('Solo un admin del grupo puede cambiar la foto.', 403);
            $path = handleImageUpload('photo', 'groups');
            $old = $pdo->prepare('SELECT photo_path FROM `groups` WHERE id=?');
            $old->execute([$gid]);
            $oldPath = $old->fetchColumn();
            $pdo->prepare('UPDATE `groups` SET photo_path=? WHERE id=?')->execute([$path, $gid]);
            if ($oldPath) @unlink(__DIR__ . '/' . $oldPath);
            ok(['photoPath' => $path]);
            break;
        }
        ...
        case 'create_meetup': {
            ...
            $actorSt = $pdo->prepare('SELECT name FROM users WHERE id=?');
            $actorSt->execute([$uid]);
            $actorName = $actorSt->fetchColumn() ?: 'Alguien';
            $groupSt = $pdo->prepare('SELECT name FROM `groups` WHERE id=?');
            $groupSt->execute([$gid]);
            $groupName = $groupSt->fetchColumn() ?: 'tu grupo';
            notifyGroupMembers($pdo, $gid, $uid, 'meetup', $actorName . ' organizó un encuentro en ' . $groupName . ': "' . $title . '"');
        ...
        case 'create_post': {
            ...
            $groupSt->execute([$gid]);
            $groupName = $groupSt->fetchColumn() ?: 'tu grupo';
            notifyGroupMembers($pdo, $gid, $uid, 'post', $actorName . ' publicó algo nuevo en ' . $groupName);
        ...
        case 'create_comment': {
            ...
            $st->execute([$pid]);
            $post = $st->fetch();
            if (!$post) fail('Ese post ya no existe.', 404);
            $groupId = (int)$post['group_id'];
            if (!isMember($pdo, $groupId, $uid)) fail('Tenés que ser miembro del grupo.', 403);

            $pdo->prepare('INSERT INTO post_comments (post_id, author_id, body) VALUES (?,?,?)')->execute([$pid, $uid, $text]);

            // Solo avisamos a quien escribió el post (no a todo el grupo, para no ser invasivos).
            if ($post['author_id'] !== null && (int)$post['author_id'] !== $uid) {
                $actorSt = $pdo->prepare('SELECT name FROM users WHERE id=?');
                $actorSt->execute([$uid]);
                $actorName = $actorSt->fetchColumn() ?: 'Alguien';
                $pdo->prepare('INSERT INTO notifications (user_id, group_id, type, actor_id, message) VALUES (?,?,?,?,?)')
                    ->execute([(int)$post['author_id'], $groupId, 'comment', $uid, $actorName . ' comentó tu publicación']);
            }
        ...
        // Se llama cuando la persona abre la campanita: marca todo como leído.
